...
var textSummary = (function (provider, translatorFactory) {
  ...
  function assembleFacets(personalityTree) {
    ...
    sentences.push(format(tphrase('You are %s'), info.term) + ' in that ' + info.description + '.');
    info = getFacetInfo(facetElements[1]);
    sentences.push(format(tphrase('You are %s'), info.term) + ' and ' + info.description + '.');
    ...
    sentences.push('And ' + info.description + '.');
    ...
    if (sameQI) {
      ...
      for (var i = 0; i < valuesInfo.length; i++) {
        ...
        sentence = sentence.concat('. ').
            concat(valuesInfo[i].description).
            concat('.');
        sentences.push(sentence);
      }
    }
